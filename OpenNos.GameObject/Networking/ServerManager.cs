...
namespace OpenNos.GameObject
{
    public class ServerManager : BroadcastableBase
    {
        ...
        public void JoinMiniland(ClientSession Session, ClientSession MinilandOwner)
        {
            ...
            Session.SendPacket(Session.Character.GenerateSay(string.Format(Language.Instance.GetMessageFromKey("MINILAND_VISITOR"), Session.Character.GeneralLogs.GetAllItems().Count(s => s.LogData == "Miniland" && s.Timestamp.Day == DateTime.Now.Day), Session.Character.GeneralLogs.GetAllItems().Count(s => s.LogData == "Miniland")), 10));
        }
        ...
        public void LeaveMap(long id)
        {
            ...
            if (!session.Character.InvisibleGm)
            {
                session.Character.Mates.Where(s => s.IsTeamMember).ToList().ForEach(s => session.CurrentMapInstance?.Broadcast(session, StaticPacketHelper.Out(UserType.Player, s.CharacterId), ReceiverType.AllExceptMe));
